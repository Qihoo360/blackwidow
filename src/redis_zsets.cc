  ...
  while (it->Valid() && (*leftover_visits) > 0) {
    ...
    if (parsed_zsets_meta_value.IsStale()
      ...
      if (min_timestamp < parsed_zsets_meta_value.timestamp()
        ...
        int len = slash::ll2string(temp_ttl, sizeof temp_ttl, parsed_zsets_meta_value.timestamp() - curtime);
        if (!len) {
          keys->push_back("0");
        } else {
...
Status RedisZSets::PKExpireReset(const int32_t min_timestamp,
  ...
  while (it->Valid()) {
    ...
    if (parsed_zsets_meta_value.IsStale()
      ...
      if (min_timestamp < parsed_zsets_meta_value.timestamp()
        && parsed_zsets_meta_value.timestamp() < max_timestamp) {
        parsed_zsets_meta_value.set_timestamp(reset_timestamp);
...
bool RedisZSets::PKFieldScan(const std::string& start_key,
  ...
  while (it->Valid() && (*leftover_visits) > 0) {
    ParsedZSetsMetaValue parsed_zsets_meta_value(it->value());
    if (!parsed_zsets_meta_value.IsStale() 
        && parsed_zsets_meta_value.count() > min_num
        && parsed_zsets_meta_value.count() < max_num) {
      keys->push_back(it->key().ToString());
