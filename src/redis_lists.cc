  ...
  *ret = 0;
  ...
  if (s.ok()) {
    ...
    if (parsed_lists_meta_value.IsStale()) {
      ...
      uint64_t next_version_index = current_index - 1;
      rocksdb::Iterator* iter =
        db_->NewIterator(default_read_options_, handles_[1]);
      ListsDataKey start_data_key(key, version, current_index);
      ListsDataKey start_next_data_key(key, version + 1, next_version_index);
      Slice start_next_prefix_key = start_next_data_key.Encode();
      rocksdb::Slice upper_bound(start_next_prefix_key);
      ...
      if (!find_pivot) {
        *ret = -1;
        ...
        if (pivot_index <= mid_index) {
          ...
          next_version_index = current_index - 1;
  ...
  if (s.ok()) {
    ...
    if (parsed_lists_meta_value.IsStale()) {
      ...
      if (sublist_left_index > sublist_right_index
        ...
        ListsDataKey start_data_key(key, version, current_index);
        ListsDataKey start_data_next_key(key, version + 1, current_index);
        Slice start_next_prefix_key = start_data_next_key.Encode();
        rocksdb::Slice upper_bound(start_next_prefix_key);
        read_options.iterate_upper_bound = &upper_bound;
        read_options.fill_cache = false;
          for (iter->Seek(start_data_key.Encode());
  ...
  *ret = 0;
  ...
  if (s.ok()) {
    ...
    if (parsed_lists_meta_value.IsStale()) {
      ...
      uint64_t next_version_index = parsed_lists_meta_value.right_index();
      ...
      Slice stop_data_prefix_key = stop_data_key.Encode();
      rocksdb::Slice upper_bound(next_version_prefix_key);
      read_options.iterate_upper_bound = &upper_bound;
      rocksdb::Slice lower_bound(stop_data_prefix_key);
      read_options.fill_cache = false;
      if (count >= 0) {
  ...
  while (it->Valid() && (*leftover_visits) > 0) {
    ...
    if (parsed_lists_meta_value.IsStale()
      ...
      if (min_timestamp < parsed_lists_meta_value.timestamp()
        ...
        int len = slash::ll2string(temp_ttl, sizeof temp_ttl, parsed_lists_meta_value.timestamp() - curtime);
        if (!len) {
          keys->push_back("0");
        } else {
...
Status RedisLists::PKExpireReset(const int32_t min_timestamp,
  ...
  while (it->Valid()) {
    ...
    if (parsed_lists_meta_value.IsStale()
      ...
      if (min_timestamp < parsed_lists_meta_value.timestamp()
        && parsed_lists_meta_value.timestamp() < max_timestamp) {
        parsed_lists_meta_value.set_timestamp(reset_timestamp);
...
bool RedisLists::PKFieldScan(const std::string& start_key,
  ...
  while (it->Valid() && (*leftover_visits) > 0) {
    ParsedListsMetaValue parsed_lists_meta_value(it->value());
    if (!parsed_lists_meta_value.IsStale()
        && parsed_lists_meta_value.count() > min_num
        && parsed_lists_meta_value.count() < max_num) { 
      keys->push_back(it->key().ToString());
      int len = slash::ll2string(temp_count, sizeof temp_count, parsed_lists_meta_value.count());
      if (!len) { 
        keys->push_back("0");
      } else { 
