  ...
  while (it->Valid() && (*leftover_visits) > 0) {
    ...
    if (parsed_sets_meta_value.IsStale()
      ...
      if (min_timestamp < parsed_sets_meta_value.timestamp()
        ...
        int len = slash::ll2string(temp_ttl, sizeof temp_ttl, parsed_sets_meta_value.timestamp() - curtime);
        if (!len) {
          keys->push_back("0");
        } else {
...
Status RedisSets::PKExpireReset(const int32_t min_timestamp,
  ...
  while (it->Valid()) {
    ...
    if (parsed_sets_meta_value.IsStale()
      ...
      if (min_timestamp < parsed_sets_meta_value.timestamp()
        && parsed_sets_meta_value.timestamp() < max_timestamp) {
        parsed_sets_meta_value.set_timestamp(reset_timestamp);
...
bool RedisSets::PKFieldScan(const std::string& start_key,
  ...
  while (it->Valid() && (*leftover_visits) > 0) {
    ParsedSetsMetaValue parsed_sets_meta_value(it->value());
    if (!parsed_sets_meta_value.IsStale()
        && parsed_sets_meta_value.count() > min_num
        && parsed_sets_meta_value.count() < max_num) {
        keys->push_back(it->key().ToString()); 
